...
| **LLM06** | Excessive Agency | ✅ | Default-deny capability scopes (fs/network/shell), allowlist-only registry, per-tool timeouts, mandatory human approval for dangerous tools, AND **EU AI Act Art. 14 DB-backed human oversight** for HIGH/CRITICAL tools (forward-only, audited, single-use, auto-expiring) — no autonomous high-risk actions. `src/ai/tools/`, `src/compliance/oversight.ts`, `tests/compliance/` |
...
| **Transparency** (EU AI Act Art. 50) | AI-generated disclosure | ✅ | Every AI response carries an `ai_meta` envelope (`ai_generated: true`, model/provider, KI-Inventar link, oversight + compliance status). `src/http/aiResponseEnvelope.ts` |
